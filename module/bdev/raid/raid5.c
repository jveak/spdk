...
struct stripe_request {
	...
	union {
		...
		struct {
			/* Buffers for data chunks that need to be read before parity calculation. */
			void **data_buffers;
			/* Buffer for the new parity. */
			void *parity_buf;
			/* Buffers for metadata of chunks that need to be read. */
			void **md_buffers;
			/* Buffer for new parity metadata */
			void *parity_md_buf;
			/* Bitmap of data chunks to read. */
			uint64_t read_chunks_mask;
			/* Number of remaining read operations. */
			uint32_t remaining_reads;
		} partial_write;
	};
...
static void raid5_xor_stripe_retry(struct stripe_request *stripe_req);

static void
raid5_xor_stripe_done(struct stripe_request *stripe_req)
{
	...
	if (!TAILQ_EMPTY(&r5ch->xor_retry_queue)) {
		...
		raid5_xor_stripe_retry(stripe_req);
	}
}

static void raid5_xor_stripe_continue(struct stripe_request *stripe_req);

static void
_raid5_xor_stripe_cb(struct stripe_request *stripe_req, int status)
{
	if (status != 0) {
	...
	if (stripe_req->xor.remaining + stripe_req->xor.remaining_md == 0) {
		raid5_xor_stripe_done(stripe_req);
	}
}

static void
raid5_xor_stripe_cb(void *_stripe_req, int status)
{
	...
	if (stripe_req->xor.remaining > 0) {
		...
		raid5_xor_stripe_continue(stripe_req);
	}

	_raid5_xor_stripe_cb(stripe_req, status);
}

static void
raid5_xor_stripe_md_cb(void *_stripe_req, int status)
{
	struct stripe_request *stripe_req = _stripe_req;
...
static void
raid5_xor_stripe_continue(struct stripe_request *stripe_req)
{
	struct raid5_io_channel *r5ch = stripe_req->r5ch;
...
static void
raid5_xor_stripe(struct stripe_request *stripe_req, stripe_req_xor_cb cb)
{
	struct raid5_io_channel *r5ch = stripe_req->r5ch;
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	...
	if (spdk_likely(stripe_req->type == STRIPE_REQ_WRITE ||
			stripe_req->type == STRIPE_REQ_PARTIAL_WRITE)) {
	...
	if (raid_io->md_buf != NULL) {
		...
		ret = spdk_accel_submit_xor(stripe_req->r5ch->accel_ch, dest_chunk->md_buf,
					    ...
					    raid5_xor_stripe_md_cb, stripe_req);
		if (spdk_unlikely(ret)) {
			if (ret == -ENOMEM) {
				TAILQ_INSERT_HEAD(&r5ch->xor_retry_queue, stripe_req, link);
			} else {
...
static void
raid5_xor_stripe_retry(struct stripe_request *stripe_req)
{
	if (stripe_req->xor.remaining_md) {
		raid5_xor_stripe(stripe_req, stripe_req->xor.cb);
	} else {
		raid5_xor_stripe_continue(stripe_req);
	}
}
...
static void
raid5_stripe_request_chunk_write_complete(struct stripe_request *stripe_req,
...
static void
raid5_stripe_request_chunk_read_complete(struct stripe_request *stripe_req,
		enum spdk_bdev_io_status status)
{
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
...
raid5_chunk_complete_bdev_io(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
	...
	if (spdk_likely(stripe_req->type == STRIPE_REQ_WRITE)) {
		raid5_stripe_request_chunk_write_complete(stripe_req, status);
	} else if (stripe_req->type == STRIPE_REQ_RECONSTRUCT) {
		raid5_stripe_request_chunk_read_complete(stripe_req, status);
	} else {
		assert(false);
	}
}

static void raid5_stripe_request_submit_chunks(struct stripe_request *stripe_req);

static void
raid5_chunk_submit_retry(void *_raid_io)
{
...
static int
raid5_chunk_submit(struct chunk *chunk)
{
	struct stripe_request *stripe_req = raid5_chunk_stripe_req(chunk);
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	...
	if (spdk_unlikely(ret)) {
		...
		if (ret == -ENOMEM) {
			raid_bdev_queue_io_wait(raid_io, spdk_bdev_desc_get_bdev(base_info->desc),
						base_ch, raid5_chunk_submit_retry);
		} else {
			/*
			...
			if (stripe_req->type == STRIPE_REQ_WRITE) {
				...
				base_bdev_io_not_submitted = raid5_stripe_data_chunks_num(raid_bdev) -
							     raid_io->base_bdev_io_submitted;
			}
...
static int
raid5_stripe_request_map_iovecs(struct stripe_request *stripe_req)
{
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	...
	FOR_EACH_DATA_CHUNK(stripe_req, chunk) {
		...
		if (raid_io->md_buf != NULL) {
			chunk->md_buf = raid_io->md_buf +
					(raid_io_offset >> r5_info->blocklen_shift) * raid_bdev->bdev.md_len;
		}
...
static inline void
raid5_stripe_request_init(struct stripe_request *stripe_req, struct raid_bdev_io *raid_io,
			   uint64_t stripe_index)
{
...
raid5_submit_write_request(struct raid_bdev_io *raid_io, uint64_t stripe_index)
{
	...
	ret = raid5_stripe_request_map_iovecs(stripe_req);
	if (spdk_unlikely(ret)) {
		return ret;
	}
...
static void
raid5_stripe_request_reconstruct_xor_done(struct stripe_request *stripe_req, int status)
{
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
...
static void
raid5_reconstruct_reads_completed_cb(struct raid_bdev_io *raid_io, enum spdk_bdev_io_status status)
{
	struct stripe_request *stripe_req = raid_io->module_private;
...
static int
raid5_submit_reconstruct_read(struct raid_bdev_io *raid_io, uint64_t stripe_index,
			       uint8_t chunk_idx, uint64_t chunk_offset, stripe_req_xor_cb cb)
{
	...
	FOR_EACH_CHUNK(stripe_req, chunk) {
		if (chunk == stripe_req->reconstruct.chunk) {
			...
			ret = raid5_chunk_set_iovcnt(chunk, raid_io->iovcnt);
			if (ret) {
				return ret;
			}
	...
	raid_io->module_private = stripe_req;
	raid_io->base_bdev_io_remaining = raid_bdev->num_base_bdevs;
	raid_io->completion_cb = raid5_reconstruct_reads_completed_cb;
			   ...
			   uint64_t stripe_offset)
{
	...
	raid5_init_ext_io_opts(&io_opts, raid_io);
	if (base_ch == NULL) {
		return raid5_submit_reconstruct_read(raid_io, stripe_index, chunk_idx, chunk_offset,
						      raid5_stripe_request_reconstruct_xor_done);
	}

	ret = raid_bdev_readv_blocks_ext(base_info, base_ch, raid_io->iovs, raid_io->iovcnt,
					 base_offset_blocks, raid_io->num_blocks,
					 raid5_chunk_read_complete, raid_io, &io_opts);
...
static void raid5_small_write_write_complete(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg);
static void raid5_small_write_xor_complete(void *cb_arg, int status);
				 ...
				 uint64_t stripe_offset)
{
	...
	struct raid5_io_channel *r5ch = raid_bdev_channel_get_module_ctx(raid_io->raid_ch);
	struct stripe_request *stripe_req;
	uint8_t p_idx, data_chunk_idx;
	uint64_t chunk_offset, base_offset_blocks;
	struct raid_base_bdev_info *data_base_info, *parity_base_info;
	struct spdk_io_channel *data_ch, *parity_ch;
	struct iovec data_iov, parity_iov;
	struct spdk_bdev_ext_io_opts io_opts;
	int ret;
	uint8_t chunk_data_idx_in_stripe;

	stripe_req = TAILQ_FIRST(&r5ch->free_stripe_requests.small_write);
	if (stripe_req == NULL) {
		return -ENOMEM;
	}
	TAILQ_REMOVE(&r5ch->free_stripe_requests.small_write, stripe_req, link);
	raid5_stripe_request_init(stripe_req, raid_io, stripe_index);
	raid_io->module_private = stripe_req;

	p_idx = stripe_req->parity_chunk->index;
	chunk_data_idx_in_stripe = stripe_offset / raid_bdev->strip_size;
	data_chunk_idx = chunk_data_idx_in_stripe < p_idx ? chunk_data_idx_in_stripe : chunk_data_idx_in_stripe + 1;

	chunk_offset = stripe_offset % raid_bdev->strip_size;
	base_offset_blocks = (stripe_index << raid_bdev->strip_size_shift) + chunk_offset;

	raid_io->base_bdev_io_remaining = 2;

	/* Read old data */
	data_base_info = &raid_bdev->base_bdev_info[data_chunk_idx];
	data_ch = raid_bdev_channel_get_base_channel(raid_io->raid_ch, data_chunk_idx);
	data_iov.iov_base = stripe_req->smallwrite.old_data;
	data_iov.iov_len = raid_io->num_blocks * raid_bdev->bdev.blocklen;
	...
	parity_iov.iov_base = stripe_req->smallwrite.old_parity;
...
static void
raid5_small_write_read_complete(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
	...
	if (--raid_io->base_bdev_io_remaining == 0) {
		raid5_small_write_calculate_new_parity_and_write(stripe_req);
	}
}

static void
raid5_small_write_calculate_new_parity_and_write(struct stripe_request *stripe_req)
{
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	struct raid5_io_channel *r5ch = stripe_req->r5ch;
	...
	assert(raid_io->iovcnt == 1); // For now, assume single iovec

	len = raid_io->num_blocks * raid_bdev->bdev.blocklen;

	srcs[0] = stripe_req->smallwrite.old_data;
	srcs[1] = stripe_req->smallwrite.old_parity;
	srcs[2] = raid_io->iovs[0].iov_base;
	new_parity_buf = stripe_req->smallwrite.old_parity; // Reuse buffer for new parity

	spdk_accel_submit_xor(r5ch->accel_ch, new_parity_buf, srcs, 3, len,
			    raid5_small_write_xor_complete, stripe_req);
}

static void
raid5_small_write_xor_complete(void *cb_arg, int status)
{
	struct stripe_request *stripe_req = cb_arg;
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	uint8_t p_idx, data_chunk_idx;
	uint64_t stripe_offset, chunk_offset, base_offset_blocks;
	struct raid_base_bdev_info *data_base_info, *parity_base_info;
	struct spdk_io_channel *data_ch, *parity_ch;
	struct iovec data_iov, parity_iov;
	struct spdk_bdev_ext_io_opts io_opts;
	...
	data_chunk_idx = chunk_data_idx_in_stripe < p_idx ? chunk_data_idx_in_stripe : chunk_data_idx_in_stripe + 1;
	chunk_offset = stripe_offset % raid_bdev->strip_size;
	base_offset_blocks = (stripe_req->stripe_index << raid_bdev->strip_size_shift) + chunk_offset;

	/* Write new data */
	data_base_info = &raid_bdev->base_bdev_info[data_chunk_idx];
	data_ch = raid_bdev_channel_get_base_channel(raid_io->raid_ch, data_chunk_idx);
	data_iov = raid_io->iovs[0];

	raid5_init_ext_io_opts(&io_opts, raid_io);
	ret = raid_bdev_writev_blocks_ext(data_base_info, data_ch, &data_iov, 1,
					  base_offset_blocks, raid_io->num_blocks,
	...
	parity_base_info = &raid_bdev->base_bdev_info[p_idx];
	parity_ch = raid_bdev_channel_get_base_channel(raid_io->raid_ch, p_idx);
	parity_iov.iov_base = stripe_req->smallwrite.old_parity; // This now holds new parity
	parity_iov.iov_len = raid_io->num_blocks * raid_bdev->bdev.blocklen;
...
static void
raid5_small_write_write_complete(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
	struct stripe_request *stripe_req = cb_arg;
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
...
static void
raid5_partial_write_xor_done(struct stripe_request *stripe_req, int status)
{
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	struct raid5_info *r5_info = raid_bdev->module_private;
	uint64_t strip_size = raid_bdev->strip_size;
	uint8_t data_chunks_num = raid5_stripe_data_chunks_num(raid_bdev);
	uint64_t stripe_offset = raid_io->offset_blocks % r5_info->stripe_blocks;
	uint64_t write_start_in_stripe = stripe_offset;
	uint64_t write_end_in_stripe = stripe_offset + raid_io->num_blocks;
	int raid_io_iov_idx = 0;
	size_t raid_io_internal_offset = 0;
	int ret;
	...
	for (uint8_t d_idx = 0; d_idx < data_chunks_num; d_idx++) {
		uint64_t chunk_start_in_stripe = d_idx * strip_size;
		uint64_t chunk_end_in_stripe = (d_idx + 1) * strip_size;
		uint64_t overlap_start = spdk_max(chunk_start_in_stripe, write_start_in_stripe);
		uint64_t overlap_end = spdk_min(chunk_end_in_stripe, write_end_in_stripe);

		if (overlap_start < overlap_end) {
			uint8_t b_idx = d_idx < stripe_req->parity_chunk->index ? d_idx : d_idx + 1;
			struct raid_base_bdev_info *base_info = &raid_bdev->base_bdev_info[b_idx];
			struct spdk_io_channel *base_ch = raid_bdev_channel_get_base_channel(raid_io->raid_ch, b_idx);
			uint64_t len_blocks = overlap_end - overlap_start;
			uint64_t offset_in_chunk = overlap_start - chunk_start_in_stripe;
			uint64_t base_offset_blocks = (stripe_req->stripe_index << r5_info->raid_bdev->strip_size_shift) +
						      offset_in_chunk;
			struct iovec iovs[raid_io->iovcnt];
			int iovcnt = 0;
			uint64_t len_bytes = len_blocks * raid_bdev->bdev.blocklen;
			uint64_t current_len = 0;

			while (current_len < len_bytes) {
				uint64_t iov_len = raid_io->iovs[raid_io_iov_idx].iov_len - raid_io_internal_offset;
				uint64_t copy_len = spdk_min(iov_len, len_bytes - current_len);

				iovs[iovcnt].iov_base = raid_io->iovs[raid_io_iov_idx].iov_base + raid_io_internal_offset;
				iovs[iovcnt].iov_len = copy_len;
				current_len += copy_len;
				raid_io_internal_offset += copy_len;
				if (raid_io_internal_offset == raid_io->iovs[raid_io_iov_idx].iov_len) {
					raid_io_iov_idx++;
					raid_io_internal_offset = 0;
				}
				iovcnt++;
			}

			raid_io->base_bdev_io_remaining++;
			ret = raid_bdev_writev_blocks_ext(base_info, base_ch, iovs, iovcnt, base_offset_blocks, len_blocks,
							  raid5_partial_write_write_complete, stripe_req, NULL);
			if (ret != 0) {
				raid_io->base_bdev_io_remaining--;
				goto err;
			}
	...
	/* Write new parity */
	uint8_t p_idx = stripe_req->parity_chunk->index;
	struct raid_base_bdev_info *parity_base_info = &raid_bdev->base_bdev_info[p_idx];
	struct spdk_io_channel *parity_ch = raid_bdev_channel_get_base_channel(raid_io->raid_ch, p_idx);
	struct iovec parity_iov;

	parity_iov.iov_base = stripe_req->partial_write.parity_buf;
...
err:
	raid5_partial_write_write_complete(NULL, false, stripe_req);
}

static void
raid5_partial_write_reads_complete(struct stripe_request *stripe_req, enum spdk_bdev_io_status status)
{
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	struct raid5_info *r5_info = raid_bdev->module_private;
	...
	if (--stripe_req->partial_write.remaining_reads != 0) {
		return;
	}

	/* All reads are complete. Now assemble the stripe data for XOR. */
	for (uint8_t d_idx = 0; d_idx < data_chunks_num; d_idx++) {
		uint8_t b_idx = d_idx < stripe_req->parity_chunk->index ? d_idx : d_idx + 1;
		struct chunk *chunk = &stripe_req->chunks[b_idx];
		void *chunk_buf = stripe_req->partial_write.data_buffers[d_idx];
		uint64_t chunk_start_in_stripe = d_idx * strip_size;
		uint64_t chunk_end_in_stripe = (d_idx + 1) * strip_size;
		uint64_t overlap_start = spdk_max(chunk_start_in_stripe, write_start_in_stripe);
		uint64_t overlap_end = spdk_min(chunk_end_in_stripe, write_end_in_stripe);
		...
		if (stripe_req->partial_write.read_chunks_mask & (1ULL << d_idx)) {
			/* This chunk was not fully overwritten. Its old content is in chunk_buf.
			 * Copy the new data into it if there is any overlap. */
			if (overlap_start < overlap_end) {
				uint64_t len_bytes = (overlap_end - overlap_start) * raid_bdev->bdev.blocklen;
				uint64_t offset_in_chunk_bytes = (overlap_start - chunk_start_in_stripe) * raid_bdev->bdev.blocklen;
				uint64_t offset_in_write_bytes = (overlap_start - write_start_in_stripe) * raid_bdev->bdev.blocklen;

				copied = spdk_iov_cpy_to_buf(chunk_buf + offset_in_chunk_bytes, len_bytes,
							     raid_io->iovs, raid_io->iovcnt,
							     offset_in_write_bytes);
				if (copied != len_bytes) {
					goto err;
				}
			}
		} else {
			/* This chunk is fully overwritten. Copy the new data into the buffer. */
			uint64_t len_bytes = strip_size * raid_bdev->bdev.blocklen;
			uint64_t offset_in_write_bytes = (chunk_start_in_stripe - write_start_in_stripe) * raid_bdev->bdev.blocklen;
			copied = spdk_iov_cpy_to_buf(chunk_buf, len_bytes,
						     raid_io->iovs, raid_io->iovcnt, offset_in_write_bytes);
			if (copied != len_bytes) {
				goto err;
			}
		}

		chunk->iovs[0].iov_base = chunk_buf;
		chunk->iovs[0].iov_len = strip_size * raid_bdev->bdev.blocklen;
		chunk->iovcnt = 1;
	}

	/* Setup parity chunk to receive the new parity data */
	stripe_req->parity_chunk->iovs[0].iov_base = stripe_req->partial_write.parity_buf;
	stripe_req->parity_chunk->iovs[0].iov_len = strip_size * raid_bdev->bdev.blocklen;
	stripe_req->parity_chunk->iovcnt = 1;

	raid5_xor_stripe(stripe_req, raid5_partial_write_xor_done);
	return;

err:
	SPDK_ERRLOG("Partial write data preparation failed\n");
	raid_bdev_io_complete(raid_io, SPDK_BDEV_IO_STATUS_FAILED);
	raid5_stripe_request_release(stripe_req);
}

static void
raid5_partial_write_read_complete_cb(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
	struct stripe_request *stripe_req = cb_arg;
	enum spdk_bdev_io_status s = success ? SPDK_BDEV_IO_STATUS_SUCCESS :
				     SPDK_BDEV_IO_STATUS_FAILED;

	spdk_bdev_free_io(bdev_io);
	raid5_partial_write_reads_complete(stripe_req, s);
}
				   ...
				   uint64_t stripe_offset)
{
	...
	uint64_t strip_size = raid_bdev->strip_size;
	uint8_t data_chunks_num = raid5_stripe_data_chunks_num(raid_bdev);
	uint64_t start_block_in_stripe = stripe_offset;
	uint64_t end_block_in_stripe = stripe_offset + raid_io->num_blocks;
	uint8_t p_idx;
	uint8_t d_idx;
	...
	stripe_req->partial_write.read_chunks_mask = 0;
	stripe_req->partial_write.remaining_reads = 0;

	for (d_idx = 0; d_idx < data_chunks_num; d_idx++) {
		uint64_t chunk_start_in_stripe = d_idx * strip_size;
		uint64_t chunk_end_in_stripe = (d_idx + 1) * strip_size;
		bool fully_overwritten = start_block_in_stripe <= chunk_start_in_stripe &&
					 end_block_in_stripe >= chunk_end_in_stripe;

		if (!fully_overwritten) {
			uint8_t b_idx = d_idx < p_idx ? d_idx : d_idx + 1;
			if (raid_bdev_channel_get_base_channel(raid_io->raid_ch, b_idx) != NULL) {
				stripe_req->partial_write.read_chunks_mask |= (1ULL << d_idx);
				stripe_req->partial_write.remaining_reads++;
			}
		}
	}

	if (stripe_req->partial_write.remaining_reads == 0) {
		raid5_partial_write_reads_complete(stripe_req, SPDK_BDEV_IO_STATUS_SUCCESS);
		return 0;
	}
	...
	for (d_idx = 0; d_idx < data_chunks_num; d_idx++) {
		if (stripe_req->partial_write.read_chunks_mask & (1ULL << d_idx)) {
			...
			iov.iov_base = stripe_req->partial_write.data_buffers[d_idx];
			iov.iov_len = strip_size * raid_bdev->bdev.blocklen;

			raid5_init_ext_io_opts(&io_opts, raid_io);
			if (raid_io->md_buf) {
				io_opts.metadata = stripe_req->partial_write.md_buffers[d_idx];
			} else {
				io_opts.metadata = NULL;
			}

			ret = raid_bdev_readv_blocks_ext(base_info, base_ch, &iov, 1,
							 stripe_index << r5_info->raid_bdev->strip_size_shift,
							 strip_size,
							 raid5_partial_write_read_complete_cb, stripe_req, &io_opts);
			if (ret != 0) {
				stripe_req->partial_write.remaining_reads = 1;
				raid5_partial_write_reads_complete(stripe_req, SPDK_BDEV_IO_STATUS_FAILED);
				return ret;
			}
		}
	}
...
raid5_stripe_request_free(struct stripe_request *stripe_req)
{
	...
	if (stripe_req->type == STRIPE_REQ_WRITE) {
		...
		if (stripe_req->partial_write.data_buffers) {
			for (i = 0; i < data_chunks_num; i++) {
				spdk_dma_free(stripe_req->partial_write.data_buffers[i]);
			}
			free(stripe_req->partial_write.data_buffers);
		}
		spdk_dma_free(stripe_req->partial_write.parity_buf);
		if (stripe_req->partial_write.md_buffers) {
			for (i = 0; i < data_chunks_num; i++) {
				spdk_dma_free(stripe_req->partial_write.md_buffers[i]);
			}
			free(stripe_req->partial_write.md_buffers);
		}
		spdk_dma_free(stripe_req->partial_write.parity_md_buf);
	} else {
		assert(false);
	}
...
raid5_stripe_request_alloc(struct raid5_io_channel *r5ch, enum stripe_request_type type)
{
	...
	if (type == STRIPE_REQ_WRITE) {
		stripe_req->write.parity_buf = spdk_dma_malloc(chunk_len, r5_info->buf_alignment, NULL);
		...
		if (raid_io_md_size != 0) {
			...
			for (i = 0; i < n; i++) {
				buf = spdk_dma_malloc(raid_bdev->strip_size * raid_io_md_size, r5_info->buf_alignment, NULL);
				if (!buf) {
					goto err;
				}
	...
	} else if (type == STRIPE_REQ_SMALL_WRITE) {
		stripe_req->smallwrite.old_data = spdk_dma_malloc(chunk_len, r5_info->buf_alignment, NULL);
		if (!stripe_req->smallwrite.old_data) {
			goto err;
		}
		stripe_req->smallwrite.old_parity = spdk_dma_malloc(chunk_len, r5_info->buf_alignment, NULL);
		...
		stripe_req->partial_write.data_buffers = calloc(data_chunks_num, sizeof(void *));
		if (!stripe_req->partial_write.data_buffers) {
			goto err;
		}
		for (i = 0; i < data_chunks_num; i++) {
			stripe_req->partial_write.data_buffers[i] = spdk_dma_malloc(chunk_len,
					r5_info->buf_alignment, NULL);
			if (!stripe_req->partial_write.data_buffers[i]) {
				goto err;
			}
		}

		stripe_req->partial_write.parity_buf = spdk_dma_malloc(chunk_len, r5_info->buf_alignment, NULL);
		if (!stripe_req->partial_write.parity_buf) {
			goto err;
		}

		if (raid_io_md_size != 0) {
			stripe_req->partial_write.md_buffers = calloc(data_chunks_num, sizeof(void *));
			if (!stripe_req->partial_write.md_buffers) {
				goto err;
			}
			for (i = 0; i < data_chunks_num; i++) {
				stripe_req->partial_write.md_buffers[i] = spdk_dma_malloc(
						raid_bdev->strip_size * raid_io_md_size, r5_info->buf_alignment, NULL);
				if (!stripe_req->partial_write.md_buffers[i]) {
					goto err;
				}
			}
			stripe_req->partial_write.parity_md_buf = spdk_dma_malloc(
					raid_bdev->strip_size * raid_io_md_size, r5_info->buf_alignment, NULL);
			if (!stripe_req->partial_write.parity_md_buf) {
				goto err;
			}
		}
	} else {
	...
	stripe_req->chunk_xor_buffers = calloc(raid5_stripe_data_chunks_num(raid_bdev),
					       sizeof(stripe_req->chunk_xor_buffers[0]));
	if (!stripe_req->chunk_xor_buffers) {
		goto err;
	}

	stripe_req->chunk_xor_md_buffers = calloc(raid5_stripe_data_chunks_num(raid_bdev),
...
raid5_ioch_destroy(void *io_device, void *ctx_buf)
{
	...
	while ((stripe_req = TAILQ_FIRST(&r5ch->free_stripe_requests.small_write))) {
		TAILQ_REMOVE(&r5ch->free_stripe_requests.small_write, stripe_req, link);
		raid5_stripe_request_free(stripe_req);
	}
...
raid5_ioch_create(void *io_device, void *ctx_buf)
{
	...
	for (i = 0; i < RAID5_MAX_STRIPES; i++) {
		stripe_req = raid5_stripe_request_alloc(r5ch, STRIPE_REQ_WRITE);
		if (!stripe_req) {
			goto err;
		}
	...
	for (i = 0; i < RAID5_MAX_STRIPES; i++) {
		stripe_req = raid5_stripe_request_alloc(r5ch, STRIPE_REQ_RECONSTRUCT);
		if (!stripe_req) {
			goto err;
		}
	...
	for (i = 0; i < RAID5_MAX_STRIPES; i++) {
		stripe_req = raid5_stripe_request_alloc(r5ch, STRIPE_REQ_SMALL_WRITE);
		if (!stripe_req) {
			goto err;
		}
...
raid5_start(struct raid_bdev *raid_bdev)
{
	...
	if (!raid_bdev->bdev.md_interleave) {
		r5_info->blocklen_shift = spdk_u32log2(raid_bdev->bdev.blocklen);
	}

	raid_bdev->bdev.blockcnt = r5_info->stripe_blocks * r5_info->total_stripes;
	raid_bdev->bdev.optimal_io_boundary = raid_bdev->strip_size;
	raid_bdev->bdev.split_on_optimal_io_boundary = true;
	raid_bdev->bdev.write_unit_size = r5_info->stripe_blocks;
...
raid5_io_device_unregister_done(void *io_device)
{
	...
	raid_bdev_module_stop_done(r5_info->raid_bdev);
...
static void
raid5_process_write_completed(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
...
static void raid5_process_submit_write(struct raid_bdev_process_request *process_req);

static void
_raid5_process_submit_write(void *ctx)
{
	struct raid_bdev_process_request *process_req = ctx;

	raid5_process_submit_write(process_req);
}

static void
raid5_process_submit_write(struct raid_bdev_process_request *process_req)
{
	struct raid_bdev_io *raid_io = &process_req->raid_io;
	struct raid_bdev *raid_bdev = raid_io->raid_bdev;
	struct raid5_info *r5_info = raid_bdev->module_private;
	uint64_t stripe_index = process_req->offset_blocks / r5_info->stripe_blocks;
	struct spdk_bdev_ext_io_opts io_opts;
	int ret;
	...
	ret = raid_bdev_writev_blocks_ext(process_req->target, process_req->target_ch,
					  ...
					  raid5_process_write_completed, process_req, &io_opts);
	if (spdk_unlikely(ret != 0)) {
		if (ret == -ENOMEM) {
			raid_bdev_queue_io_wait(raid_io, spdk_bdev_desc_get_bdev(process_req->target->desc),
						process_req->target_ch, _raid5_process_submit_write);
		} else {
...
static void
raid5_process_stripe_request_reconstruct_xor_done(struct stripe_request *stripe_req, int status)
{
	struct raid_bdev_io *raid_io = stripe_req->raid_io;
...
static int
raid5_submit_process_request(struct raid_bdev_process_request *process_req,
			      struct raid_bdev_io_channel *raid_ch)
{
	...
	assert((process_req->offset_blocks % r5_info->stripe_blocks) == 0);

	if (process_req->num_blocks < r5_info->stripe_blocks) {
		return 0;
	}
	...
	ret = raid5_submit_reconstruct_read(raid_io, stripe_index, chunk_idx, 0,
					     raid5_process_stripe_request_reconstruct_xor_done);
	if (spdk_likely(ret == 0)) {
		return r5_info->stripe_blocks;
	} else if (ret < 0) {
		return ret;
	} else {
		return -EINVAL;
...
static struct raid_bdev_module g_raid5_module = {
	...
	.submit_process_request = raid5_submit_process_request,
};
